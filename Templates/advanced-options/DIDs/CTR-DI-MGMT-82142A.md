...
### 5. Overview of Test Results

#### 5.1 Overall Assessment of Hardware/Software Tested
...
- **5.1.3.1** Its impact on hardware/software and system security posture, including identification of security requirements not met
- **5.1.3.2** The impact on hardware/software and system design to correct it
- **5.1.3.3** A recommended solution/approach for correcting it
...
#### 6.2 (Project-Unique Identifier of a Test Case with Problems)
For each test case in which one or more problems occurred, provide:
- **6.2.1** A brief description of the result(s) that occurred
- **6.2.2** Identification of the test procedure step(s) in which they occurred
- **6.2.3** Reference(s) to the associated problem/change report(s) and backup data, as applicable
- **6.2.4** The number of times the procedure or step was repeated in attempting to correct the problem(s) and the outcome of each attempt
- **6.2.5** Back-up points or test steps where tests were resumed for retesting

#### 6.3 Deviations from Test Cases/Procedures
Divided into subparagraphs identifying each test case in which deviations from test case/test procedures occurred.

#### 6.4 (Project-Unique Identifier of a Test Case with Deviations)
For each test case in which one or more deviations occurred, provide:
- **6.4.1** A description of the result(s) (e.g., test case run in which the deviation occurred, nature of the deviation such as substitution of required equipment, procedural steps not followed, schedule deviations). Red-lined test procedures may be used to show the deviations.
- **6.4.2** The rationale for the deviation(s) if required
- **6.4.3** An assessment of the deviations' impact on the validity of the test case
...
- **7.1** The date(s), time(s), and location(s) of the tests performed
- **7.2** The hardware and software configurations used for each test, including as applicable: part/model/serial number, manufacturer, revision level, calibration date of all hardware; version number and name for software components used
- **7.3** The date and time of each test-related activity, the identity of the individual(s) who performed the activity, and the identities of witnesses, as applicable

---

### 8. Red Lines, Notes, and Unplanned Tests
...
- **Generate** a CSTR by reading test execution records from the project's cybersecurity testing workspace and mapping each test to a §6.1 section, with problems in §6.2 and deviations in §6.4
- **Validate** a draft CSTR for completeness — §5.1.3 requires that every finding/deficiency include all three sub-fields (security posture impact, design correction impact, and recommended solution); §6.2 requires all five fields per problem case; §6.4 requires all three fields per deviation case
- **Map to the CTP and CSTP/CSTD:** test identifiers in §6.1 must match those in CTP §3; test case identifiers in §6.2/6.4 must match CSTP/CSTD §2.1; step references in §6.2.2 must match CSTP/CSTD §4.1 step numbers
- **Populate §8 (Red Lines):** this section is unique to the CSTR — it requires scanned handwritten tester materials as a document artifact, not structured data. Flag this requirement when generating the CSTR so the project can collect physical or scanned materials from testers.
- **Map to the RTVM:** security requirement identifiers referenced in §5.1.3.1 (requirements not met) are the same identifiers tracked in the RTVM (DI-MGMT-82133A) — link findings to RTVM rows with "Non-Compliant" or "Partial Compliant" status and non-conforming entries pointing to problem/change reports

When generating a CSTR, begin with §5 (overall assessment) as the executive summary, then expand each test in §6 from lowest-level execution records, then compile the §7 test log from chronological activity data.
