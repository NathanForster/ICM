...
| `.icm-runner.py` | Runs a single isolated pipeline stage. Assembles the LLM context by walking up from the stage folder to the nearest ancestor holding `ICM.md` (the project root), loading root `ICM.md`/`CONTEXT.md` (Layers 0/1), any intermediate workspace `ICM.md`/`CONTEXT.md` (Layer 1a), and the stage `CONTEXT.md` (Layer 2) |
...
| Scripts | Copy to the instance root; edit `run_data_pipeline.sh`'s per-stage blocks and `run_source_dev.sh`'s workspace argument to the project's real folders; adjust `check_requirements.py` default paths if the register/brief folders differ from the sys-eng layout |
